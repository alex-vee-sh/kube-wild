...
// Pool for reusing K8sItemPartial structs during JSON parsing
var itemPool = sync.Pool{
	New: func() interface{} {
		return &K8sItemPartial{}
	},
}
...
// K8sItemPartial is a single item for streaming JSON parsing
type K8sItemPartial struct {
	Metadata struct {
		Name              string            `json:"name"`
		Namespace         string            `json:"namespace"`
		CreationTimestamp string            `json:"creationTimestamp"`
...
func discoverNames(runner Runner, resource string, discoveryFlags []string) ([]NameRef, error) {
	...
	// Use streaming JSON decoder for better performance with large lists
	return parseK8sListStreaming(out)
}

// parseK8sListStreaming uses a streaming JSON decoder to parse items one at a time.
// This is more memory-efficient than json.Unmarshal for large lists because it
// doesn't need to hold the entire parsed structure in memory.
func parseK8sListStreaming(data []byte) ([]NameRef, error) {
	if len(data) == 0 {
		return nil, fmt.Errorf("empty response from kubectl")
	}

	// Fast path: find "items" position using byte scanning
	// This is faster than token-by-token parsing for the initial structure
	itemsStart := bytes.Index(data, []byte(`"items":`))
	if itemsStart == -1 {
		itemsStart = bytes.Index(data, []byte(`"items" :`))
	}
	if itemsStart == -1 {
		return nil, fmt.Errorf("no items array found in kubectl output")
	}

	// Find the [ after "items":
	arrayStart := bytes.IndexByte(data[itemsStart:], '[')
	if arrayStart == -1 {
		return nil, fmt.Errorf("no items array found in kubectl output")
	}
	arrayStart += itemsStart

	// Create decoder starting from the array
	dec := json.NewDecoder(bytes.NewReader(data[arrayStart:]))

	// Consume the opening [
	if _, err := dec.Token(); err != nil {
		return nil, fmt.Errorf("failed to parse items array: %w", err)
	}

	// Pre-allocate with estimated capacity
	estimatedItems := len(data) / 300
	if estimatedItems < 10 {
		estimatedItems = 10
	}
	refs := make([]NameRef, 0, estimatedItems)

	// Stream through array items using pooled structs
	for dec.More() {
		it := itemPool.Get().(*K8sItemPartial)
		// Reset fields that might have data from previous use
		it.Metadata.Name = ""
		it.Metadata.Namespace = ""
		it.Metadata.CreationTimestamp = ""
		it.Metadata.Labels = nil
		it.Metadata.Annotations = nil
		it.Metadata.OwnerReferences = nil
		it.Spec = nil
		it.Status = nil
		...
		refs = append(refs, processItem(it))
		itemPool.Put(it)
	}
...
// processItem converts a K8sItemPartial to a NameRef
func processItem(it *K8sItemPartial) NameRef {
	...
	// Performance: pre-allocate slices with estimated capacity
	reasons := make([]string, 0, 4)
	var phase string
	totalRestarts := 0
	notReady := 0
	var reasonsByContainer map[string][]string
	...
	if it.Status != nil {
		...
		if len(it.Status.ContainerStatuses) > 0 {
			reasonsByContainer = make(map[string][]string, len(it.Status.ContainerStatuses))
		}
	...
	// Performance: pre-allocate owners slice
	owners := make([]string, 0, len(it.Metadata.OwnerReferences))
